...
const handleSubmitForm = async(e)=>{
    ...
    try{
        ...
        setFormLoad(true);
        setFormError(false);
        const SendData = await fetch('http://localhost:3100/api/create',{
            method:'POST',
            credentials:'include',
            headers:{
  ...
  return (
    <main  className="p-5 mx-auto">
        <h1 className='text-4xl my-10 font-serif font-semibold text-center text-head'>Create a new House Listing </h1>
        <form onSubmit={handleSubmitForm} className='flex mx-10 flex-col sm:flex-row'>
            ...
            <button disabled={formload || uploading} className='p-2 bg-metal w-96 text-white rounded-lg uppercase hover:opcaity-90 disabled:opacity-50'>{formload?"creating listing..." : "Create Listing"}</button>
